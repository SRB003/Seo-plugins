 ...
 */
export async function analyzeCompetitorPage(url) {
 ...
 */
export function generateModifierSuggestions(seed) {
    const prefixes = [
 ...
 */
export function generateQuestionVariations(seed) {
    const patterns = [
        `what is ${seed}`,
        `how does ${seed} work`,
        `how to use ${seed}`,
    ...
    return patterns.map((kw) => ({ keyword: kw, source: "modifier" }));
}
/**
 ...
 */
export function classifyFunnelStage(intent) {
 ...
 */
export function estimateDifficulty(competitorPages, keyword) {
    ...
    const avgWordCount = competitorPages.reduce((s, p) => s + p.estimatedWordCount, 0) / competitorPages.length;
    ...
    if (schemaRate > 0.5)
        score += 15;
    else if (schemaRate > 0.2)
        score += 8;
    // Internal linking depth — more links = more authority
    const avgInternalLinks = competitorPages.reduce((s, p) => s + p.internalLinks, 0) / competitorPages.length;
    if (avgInternalLinks > 50)
        score += 20;
    else if (avgInternalLinks > 20)
        score += 10;
    ...
    const difficulty = score <= 25 ? "easy" : score <= 50 ? "medium" : score <= 75 ? "hard" : "very-hard";
    return {
        keyword,
        difficulty,
        score,
 ...
 */
export function extractSerpFeatures(html) {
    const $ = cheerio.load(html);
    const peopleAlsoAsk = [];
...
//# sourceMappingURL=serp-analyzer.js.map