 ...
 */
export function scoreContent(content, primaryKeyword, secondaryKeywords = [], targetWordCount, metaTitle, metaDescription, slug) {
    ...
    if (metaTitle) {
        if (metaTitle.length <= 60 && metaTitle.toLowerCase().includes(lowerKeyword)) {
            ...
            if (metaTitle.length > 60)
                issues.technicalSeo.push(`Meta title too long (${metaTitle.length} chars)`);
            if (!metaTitle.toLowerCase().includes(lowerKeyword))
                issues.technicalSeo.push("Meta title missing primary keyword");
        }
    }
    else {
...
//# sourceMappingURL=content-scorer.js.map