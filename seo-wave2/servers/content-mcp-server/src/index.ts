...
import {
  ...
  generateModifierSuggestions,
  generateQuestionVariations,
  classifyIntent,
  classifyFunnelStage,
  estimateDifficulty,
  extractSerpFeatures,
} from "./services/serp-analyzer.js";
...
Returns: SERP analysis with top results, PAA, related searches, and competitor content breakdown.`,
  ...
  async ({ keyword, num_results }) => {
    try {
      ...
      let serpHtml: string;
      let organicResults: { url: string; title: string; description: string }[] = [];
      ...
      if (serpApiKey) {
        ...
        const organic = (apiData.organic_results || []) as Array<{ link: string; title: string; snippet: string }>;
      ...
      const wordCounts = detailedResults
        .filter((r): r is typeof r & { estimatedWordCount: number } => "estimatedWordCount" in r)
...
Returns: List of topics the competitor covers that are missing from your site.`,
  ...
  async ({ your_site, competitor_site, max_pages }) => {
    try {
      ...
      async function crawlTopics(siteUrl: string, maxPages: number): Promise<string[]> {
        ...
        while (queue.length > 0 && topics.length < maxPages) {
          ...
          try {
            ...
            if (h1) topics.push(h1.toLowerCase());
            else if (title) topics.push(title.toLowerCase());
...
Returns: Detailed score breakdown with category scores, issues, and strengths.`,
  ...
  async ({ content, primary_keyword, secondary_keywords, target_word_count, meta_title, meta_description, slug }) => {
    try {
      ...
      const allIssues = [
        ...result.categories.keywordOptimization.issues.map((i) => ({ category: "Keyword", issue: i, severity: "important" as const })),
        ...result.categories.contentStructure.issues.map((i) => ({ category: "Structure", issue: i, severity: "important" as const })),
        ...result.categories.readability.issues.map((i) => ({ category: "Readability", issue: i, severity: "important" as const })),
        ...result.categories.technicalSeo.issues.map((i) => ({ category: "Technical", issue: i, severity: "important" as const })),
        ...result.categories.contentQuality.issues.map((i) => ({ category: "Quality", issue: i, severity: "nice-to-have" as const })),
        ...result.categories.engagement.issues.map((i) => ({ category: "Engagement", issue: i, severity: "nice-to-have" as const })),
      ];
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            recommendation:
              result.totalScore >= 90 ? "Excellent — ready to publish." :
