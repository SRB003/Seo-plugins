...
export async function checkRanking(
  ...
  const topCompetitors: RankingResult["topCompetitors"] = [];
  ...
  if (serpApiKey) {
    ...
    if ((data.related_questions as unknown[])?.length) serpFeatures.push("people_also_ask");
 ...
 */
export async function checkAlgorithmUpdates(daysBack: number = 30): Promise<AlgorithmUpdate[]> {
  ...
  const unique = updates.filter(
    (u, i) => updates.findIndex((u2) => u2.name.toLowerCase().includes(u.name.toLowerCase().slice(0, 20))) === i
  );

  return unique.slice(0, 10);
}

function extractUpdateTargets(text: string): string[] {
  const targets: string[] = [];
  const lower = text.toLowerCase();
  if (lower.includes("core")) targets.push("core_ranking");
  if (lower.includes("content") || lower.includes("helpful")) targets.push("content_quality");
  if (lower.includes("spam") || lower.includes("link")) targets.push("spam_links");
  if (lower.includes("local")) targets.push("local_search");
  if (lower.includes("product") || lower.includes("review")) targets.push("product_reviews");
  if (lower.includes("e-e-a-t") || lower.includes("eeat") || lower.includes("expertise")) targets.push("eeat");
...
export async function findLinkProspects(
  niche: string,
  strategy: LinkProspect["strategy"],
  ...
  for (const query of searchQueries) {
    ...
    try {
      ...
      $(".g a, .tF2Cxc a").each((_, el) => {
        if (prospects.length >= maxResults) return;
        const href = $(el).attr("href") || "";
        if (!href.startsWith("http")) return;
        ...
        try {
          ...
          if (prospects.some((p) => p.domain === domain)) return;

          // Basic quality signals
          const relevanceSignals: string[] = [];
          if (title.toLowerCase().includes(niche.toLowerCase())) relevanceSignals.push("niche_in_title");
 ...
 */
export async function findBrokenLinksOnSite(
  siteUrl: string,
  maxPages: number = 10
): Promise<BrokenLinkOnSite[]> {
  const broken: BrokenLinkOnSite[] = [];
  ...
  while (queue.length > 0 && visited.size < maxPages) {
    ...
    try {
      ...
      const linksToCheck: { href: string; anchor: string }[] = [];
      $("a[href]").each((_, el) => {
        const href = $(el).attr("href");
