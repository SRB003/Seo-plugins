...
import {
  checkRanking,
  checkAlgorithmUpdates,
  findLinkProspects,
  findBrokenLinksOnSite,
} from "./services/monitor-utils.js";
...
server.registerTool(
  "monitor_check_algorithm_updates",
  {
...
server.registerTool(
  "monitor_find_broken_links_on_site",
  {
...
Returns: List of pages linking to the domain with their anchor text and source domain.`,
  ...
  async ({ domain, max_results }) => {
    try {
      ...
      const referrers: Map<string, { url: string; title: string; domain: string }> = new Map();

      for (const query of queries) {
        if (referrers.size >= max_results) break;
        try {
          const serpApiKey = process.env.SERP_API_KEY;
          let html: string;
