...
Returns: Score breakdown per technique with specific improvement recommendations.`,
    ...
    try {
        ...
        const externalLinks = $("a[href^='http']").filter((_, el) => {
            try {
                return new URL($(el).attr("href") || "").hostname !== new URL(url).hostname;
            }
            catch {
                return false;
            }
        ...
        if (citationScore < 12)
            issues.push(`Low citation density (${externalLinks.length} citations in ${totalWords} words). Target ${GEO_CONSTANTS.MIN_CITATIONS_PER_500_WORDS}+ per 500 words.`);
        else
            strengths.push(`Good citation density: ${externalLinks.length} external references`);
        ...
        if (attributionPatterns >= 3)
            strengths.push("Strong source attribution language");
        else
            issues.push("Add attribution phrases like 'According to [Source]' for cited facts");
        // 2. Statistics (20 pts)
        ...
        if (statsScore < 12)
            issues.push(`Low statistic density (${statMatches.length} data points). Add specific numbers with sources.`);
        else
            strengths.push(`Strong data density: ${statMatches.length} statistics/data points`);
        // 3. Quotations (15 pts)
        ...
        if (quoteScore < 6)
            issues.push("No expert quotes found. Add attributed quotes from industry experts.");
        else
            strengths.push(`${quoteMatches.length} expert quotes with attribution`);
        // 4. Fluency (25 pts)
        ...
        if (avgSentenceLength > 22) {
            fluencyScore -= 8;
            issues.push(`Average sentence too long (${avgSentenceLength.toFixed(1)} words). Target 15-18.`);
        }
        else if (avgSentenceLength > 18) {
            fluencyScore -= 3;
        }
        else {
            strengths.push(`Clean sentence length: avg ${avgSentenceLength.toFixed(1)} words`);
        }
        if (hedgingWords > 5) {
            fluencyScore -= 5;
            issues.push(`${hedgingWords} hedging words found. Replace with definitive language.`);
        }
        if (passiveVoice > sentences.length * 0.3) {
            fluencyScore -= 5;
            issues.push("High passive voice usage. Switch to active voice.");
        }
        ...
        if (termScore < 8)
            issues.push("Low technical terminology. Use precise domain terms where appropriate.");
        else
...
Returns: robots.txt analysis showing which AI bots are blocked/allowed, plus page accessibility check.`,
    ...
    try {
        ...
        if (robotsTxt) {
            ...
            for (const bot of GEO_CONSTANTS.AI_BOT_USER_AGENTS) {
                if (!blocked.includes(bot) && !allowed.includes(bot))
                    allowed.push(bot);
            }
        }
        else {
