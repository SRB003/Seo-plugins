...
Returns: Extracted profile data with completeness scoring.`,
  ...
  try {
    ...
    if (serpApiKey) {
      ...
      const local = (data.local_results || []) as Array<Record<string, unknown>>;
...
Returns: Local pack results with business names, ratings, review counts, and features.`,
  ...
  try {
    ...
    if (serpApiKey) {
      ...
      const res = await fetch(apiUrl);
      const data = (await res.json()) as Record<string, unknown>;
      const localPack = (data.local_results || data.local_map || {}) as Record<string, unknown>;
      return {
        content: [{ type: "text", text: JSON.stringify({
          keyword, city,
    ...
    const $ = cheerio.load(html);
    const results: { name: string; info: string }[] = [];
...
Returns: List of directory listings found with NAP match status.`,
  ...
  try {
    ...
    const results: { directory: string; tier: number; found: boolean; url: string; matchDetails: string }[] = [];
...
Returns: List of recent reviews with ratings, text, and response status.`,
  ...
  try {
    ...
    if (serpApiKey) {
      ...
      const mapsData = (await mapsRes.json()) as Record<string, unknown>;
      const localResults = (mapsData.local_results || []) as Array<Record<string, unknown>>;
