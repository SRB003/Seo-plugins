...
import { z } from "zod";
import { fetchPage, parsePageSEO, extractCrawlableUrls, checkUrl, fetchRobotsTxt, fetchSitemap, } from "./services/crawler.js";
...
// Tool 1: Crawl Site
// ─────────────────────────────────────────────
server.registerTool("seo_crawl_site", {
...
// ─────────────────────────────────────────────
server.registerTool("seo_audit_meta_tags", {
...
// Tool 3: Audit Images
// ─────────────────────────────────────────────
server.registerTool("seo_audit_images", {
...
// Tool 4: Check Links
// ─────────────────────────────────────────────
server.registerTool("seo_check_links", {
...
Returns: List of broken links with their status codes.`,
    ...
    try {
        ...
        for (const link of internalLinks.slice(0, 100)) {
            ...
            if (!ok)
                brokenInternal.push({ href: link, status });
        ...
        if (check_external) {
            for (const link of externalLinks.slice(0, 50)) {
                const { status, ok } = await checkUrl(link);
                if (!ok)
                    brokenExternal.push({ href: link, status });
                await new Promise((r) => setTimeout(r, 100));
            }
        }
        const result = {
...
// ─────────────────────────────────────────────
server.registerTool("seo_check_core_web_vitals", {
...
Returns: Core Web Vitals scores with ratings (good/needs-improvement/poor).`,
    ...
    try {
        ...
        const result = {
            ...
            overall_performance_score: lighthouse?.categories?.performance?.score
...
// ─────────────────────────────────────────────
server.registerTool("seo_check_schema", {
...
// ─────────────────────────────────────────────
server.registerTool("seo_check_sitemap", {
...
Returns: Sitemap status, URL count, robots.txt directives, and any issues.`,
    ...
    try {
        ...
        if (sitemapUrls.length === 0)
            issues.push("No sitemap.xml found or it's empty");
        if (!robotsTxt)
...
// ─────────────────────────────────────────────
server.registerTool("seo_full_audit_score", {
