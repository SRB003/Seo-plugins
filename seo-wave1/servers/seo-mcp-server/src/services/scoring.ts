import { SEO_CONSTANTS, type AuditScore, type CrawlResult, type MetaAuditResult } from "../types.js";

/**
 * Audit meta tags for a single page and return structured issues
 */
export function auditMetaTags(page: CrawlResult): MetaAuditResult {
  ...
  if (!page.title) {
    ...
    if (page.title.length < SEO_CONSTANTS.TITLE_MIN_LENGTH) titleIssues.push(`Too short (${page.title.length} chars, min ${SEO_CONSTANTS.TITLE_MIN_LENGTH})`);
    if (page.title.length > SEO_CONSTANTS.TITLE_MAX_LENGTH) titleIssues.push(`Too long (${page.title.length} chars, max ${SEO_CONSTANTS.TITLE_MAX_LENGTH})`);
  }
  ...
  if (!page.metaDescription) {
    ...
    if (page.metaDescription.length < SEO_CONSTANTS.DESCRIPTION_MIN_LENGTH) descIssues.push(`Too short (${page.metaDescription.length} chars, min ${SEO_CONSTANTS.DESCRIPTION_MIN_LENGTH})`);
    if (page.metaDescription.length > SEO_CONSTANTS.DESCRIPTION_MAX_LENGTH) descIssues.push(`Too long (${page.metaDescription.length} chars, max ${SEO_CONSTANTS.DESCRIPTION_MAX_LENGTH})`);
  }
  ...
  // Robots meta checks
  if (page.robots) {
    if (page.robots.includes("noindex")) robotsIssues.push("Page is set to noindex");
    if (page.robots.includes("nofollow")) robotsIssues.push("Page is set to nofollow");
  }
 ...
 */
export function calculateAuditScore(pages: CrawlResult[]): AuditScore {
  if (pages.length === 0) {
  ...
  const overall = Math.round(
    crawlability * 0.25 + onPage * 0.25 + performance * 0.2 + contentSignals * 0.15 + structuredData * 0.1 + imageScore * 0.05
  );
 ...
 */
export function issueSeverity(issue: string): "critical" | "important" | "nice-to-have" {
  ...
  if (critical.some((c) => issue.includes(c))) return "critical";
  if (important.some((i) => issue.includes(i))) return "important";
  return "nice-to-have";
}
