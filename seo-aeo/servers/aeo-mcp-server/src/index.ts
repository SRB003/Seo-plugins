...
Returns: AEO score breakdown with specific issues and recommendations.`,
  ...
  try {
    ...
    if (answerFirstScore < 3) issues.push("Most sections don't lead with a concise answer paragraph (40-60 words)");
    else strengths.push("Good answer-first structure");
    ...
    if (questionScore < 3) issues.push("H2 headings aren't phrased as questions — rephrase to match user prompts");
    else strengths.push("Question-format headings present");
    ...
    if (factScore < 3) issues.push(`Low fact density (${numberMatches.length} data points in ${words.length} words). Add specific stats.`);
    else strengths.push(`Good fact density: ${numberMatches.length} data points`);
    ...
    if (citationScore < 2) issues.push("Few external source citations — AI engines value well-sourced content");
    else strengths.push(`${externalLinks.length} external source citations`);

    // 5. E-E-A-T signals
    let eeatScore = 0;
    const hasAuthor = $('[rel="author"], .author, .byline, [itemprop="author"]').length > 0;
    if (hasAuthor) { eeatScore += 2; strengths.push("Author attribution found"); }
    else issues.push("No author byline detected — add author + credentials");
    const hasDate = $('time, [datetime], .published-date, .updated-date, meta[property="article:modified_time"]').length > 0;
    if (hasDate) eeatScore += 1;
    else issues.push("No publish/update date found — add visible date");
    ...
    schemas.each((_, el) => {
      try {
        ...
        if (["FAQPage", "HowTo", "QAPage"].includes(type)) schemaScore += 2;
        else if (["Article", "BlogPosting"].includes(type)) schemaScore += 1;
        if (parsed.speakable) schemaScore += 1;
      } catch { /* skip */ }
    });
    schemaScore = Math.min(4, schemaScore);
    if (schemaScore < 2) issues.push("Missing AEO-optimized schema (FAQ, HowTo, or Speakable)");
    else strengths.push("AEO-relevant schema markup found");
    ...
    for (const keyword of keywords) {
      try {
        ...
        if (serpApiKey) {
          ...
          snippetData = (data.answer_box || data.featured_snippet || {}) as Record<string, unknown>;
          const paa = (data.related_questions || []) as Array<{ question: string }>;
        ...
        const snippetUrl = (snippetData.link || snippetData.displayed_link || "") as string;
    ...
    for (const query of queries) {
      try {
        ...
        if (serpApiKey) {
          ...
          if (aiMention) context = "Mentioned in Google AI Overview/Answer Box";
          ...
          if (aiMention) context = "Brand name found in search results page";
        }
