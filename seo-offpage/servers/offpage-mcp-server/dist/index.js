...
import { z } from "zod";
import { discoverBacklinks, assessToxicity, analyzeAnchorText, findUnlinkedMentions, findLinkIntersections, findCompetitorTopContent, generateDisavowFile, discoverPROpportunities, } from "./services/offpage-utils.js";
...
// ─────────────────────────────────────────────
server.registerTool("offpage_audit_backlinks", {
...
function getAnchorHealthCheck(distribution, total) {
    if (total === 0)
        return "No backlinks found to analyze.";
    const brandedPct = ((distribution.branded || 0) / total) * 100;
    const exactPct = ((distribution.exact_match || 0) / total) * 100;
    const issues = [];
    if (brandedPct < 20)
        issues.push("Low branded anchor ratio (< 20%) — consider more brand-building");
    if (exactPct > 30)
        issues.push("High exact-match anchor ratio (> 30%) — risk of over-optimization penalty");
    if (brandedPct > 70)
        issues.push("Very high branded ratio — may benefit from more topical anchors");
    return issues.length > 0 ? issues.join(". ") : "Anchor text distribution looks healthy.";
}
// ─────────────────────────────────────────────
// Tool 2: Competitor Backlink Analysis
// ─────────────────────────────────────────────
server.registerTool("offpage_competitor_backlinks", {
...
// ─────────────────────────────────────────────
server.registerTool("offpage_find_unlinked_mentions", {
...
// ─────────────────────────────────────────────
server.registerTool("offpage_find_link_intersections", {
...
// ─────────────────────────────────────────────
server.registerTool("offpage_discover_pr_opportunities", {
