...
export async function discoverBacklinks(
  ...
  const referrers: Map<string, ReferrerEntry> = new Map();
  ...
  for (const query of queries) {
    if (referrers.size >= maxResults) break;
    try {
      if (serpApiKey) {
...
function extractAnchorFromSnippet(snippet: string, domain: string): string {
  if (snippet.toLowerCase().includes(domain)) return domain;
...
export function assessToxicity(referrers: ReferrerEntry[]): ToxicLink[] {
...
export function analyzeAnchorText(referrers: ReferrerEntry[], brandName: string): AnchorTextEntry[] {
  const anchorCounts = new Map<string, { count: number; type: AnchorTextEntry["type"] }>();
  const brandLower = brandName.toLowerCase();

  for (const ref of referrers) {
    const anchor = ref.anchorText.toLowerCase().trim() || "unknown";
    let type: AnchorTextEntry["type"] = "other";
    ...
    try {
      ...
      for (const r of results) {
        try {
          ...
          if (pageDomain.includes(cleanDomain)) continue;
          if (mentions.some((m) => m.pageUrl === r.url)) continue;
...
function calculateMentionScore(title: string, snippet: string, brandName: string): number {
  let score = 50;
  const lower = (title + " " + snippet).toLowerCase();
  const brand = brandName.toLowerCase();

  // Direct brand mention in title
  if (title.toLowerCase().includes(brand)) score += 15;
  // Multiple mentions
  const mentionCount = (lower.match(new RegExp(brand, "gi")) || []).length;
  if (mentionCount > 1) score += 10;
  // Positive context
  if (/recommend|best|top|review|guide|resource/i.test(lower)) score += 10;
  // Editorial content (more likely to add a link)
  if (/blog|article|post|guide|review/i.test(lower)) score += 10;
  // Negative context (less likely to cooperate)
  if (/complaint|scam|avoid|worst/i.test(lower)) score -= 20;
...
export async function findLinkIntersections(
  yourDomain: string,
  competitorDomains: string[],
  serpApiKey?: string,
  maxResults: number = 30
): Promise<LinkIntersection[]> {
  const cleanYourDomain = normalizeDomain(yourDomain);
  const cleanCompetitors = competitorDomains.map(normalizeDomain);
  const intersections: Map<string, LinkIntersection> = new Map();
  ...
  for (const competitor of cleanCompetitors) {
    ...
    for (const link of competitorLinks) {
      ...
      if (refDomain.includes(cleanYourDomain)) continue;
      if (cleanCompetitors.some((c) => refDomain.includes(c))) continue;
...
function generatePitchAngle(title: string, competitor: string): string {
...
export async function findCompetitorTopContent(
  competitorDomain: string,
  serpApiKey?: string,
  maxResults: number = 10
): Promise<Array<{ url: string; title: string; estimatedBacklinks: number; topReferrers: string[] }>> {
  const cleanDomain = normalizeDomain(competitorDomain);
  const contentMap = new Map<string, { title: string; referrers: Set<string> }>();
...
export function generateDisavowFile(toxicLinks: ToxicLink[]): string {
...
export async function discoverPROpportunities(
  niche: string,
  brandName: string,
  serpApiKey?: string
): Promise<Array<{ type: string; topic: string; angle: string; source: string }>> {
  const opportunities: Array<{ type: string; topic: string; angle: string; source: string }> = [];
  ...
  for (const query of queries) {
    try {
      ...
      for (const r of results) {
        ...
        if (/study|research|data|survey|report/i.test(r.title)) type = "data_story";
        if (/breaking|just|now|today/i.test(r.title)) type = "newsjack";
        if (/expert|comment|quote|looking for/i.test(r.title)) type = "expert_commentary";
        if (/query|source|journalist/i.test(r.title)) type = "haro_query";
